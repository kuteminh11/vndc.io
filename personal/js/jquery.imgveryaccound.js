;(function($, window, document, undefined) {
    ...
    Plugin.prototype = {
        ...
        handleFileUpload: function(file) {
            ...
            var number_images = document.getElementById('number_images').value;
            var d = new Date();
            var n = d.getTime();
            var file_name = login_userid+n+'.'+ext;

            fdata.append('file',file);
            fdata.append('folder','vndc');
            fdata.append('file_name', file_name);
            ext = ext.toLowerCase();
            if(ext == 'jpg' || ext == 'gif' || ext == 'jpeg' || ext == 'png'){
				if (Number(file_size) <= 5242880) {
					$.ajax({
						url:'https://image.trustpay.vn/post_v2.php',
						method:'post',
						data:fdata,
						dataType : "json",
						contentType: false,
						...
						success: function(url) {
						 //console.log(url);
						 if(url != "" ){
							$("#fileinput-filename"+number_images).html('' +
											'<div style="height: auto;overflow: hidden;position: relative">' +
											'<img style="width:100%;height:100%;margin-right:10px;max-width:400px;" src="'+ url+'?lastmod='+n +'">' +
											'<a onclick="console.log($(this).parent().remove());" style="color: red;cursor: pointer;position: absolute;top: 15px;right: 15px;font-weight: bold;" >X</a>' +
											'</div>');

							$('#txtFile'+number_images).val(url);                      
						 }
						},/*$(this).parent().remove();removethis('+number_images+')*/
					});
				}
				else {
					swal("Thông báo", "Dung lượng tối đa là 5MB", "error");
				}                
            }else{
                swal("Thông báo", "Định dạng hình ảnh không đúng", "error");
            }
        },
